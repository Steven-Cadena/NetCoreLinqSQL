...
namespace NetCoreLinqSQL.Data
{
    public class EmpleadosContext
    {
        ...
        {
            //PARA RECUPERAR DATOS CON LINQ, NECESITAMOS HACERLO SOBRE LA COLECCION DE FILAS DE LA TABLA
            var consulta = from datos in this.tablaemp.AsEnumerable()
                           select datos;
            //AHORA MISMO EN CONSULTA TENEMOS UNA COLECCION DE FILAS QUE TIENE CADA FILA UN CAMPO (FIELD)
        ...
        public Empleado FindEmpleado(int idempleado) 
        {
            var consulta = from datos in this.tablaemp.AsEnumerable()
                           ...
                           select datos;
            //TENEMOS UN METODO PARA RECUPERAR EÑ PRIMER REGISTRO 
        ...
        public List<Empleado> GetEmpleadosOficioSalario(string oficio, int salario)
        {
            ...
            if (consulta.Count() == 0)
            {
                //si son objetos si no devuelve nada siempre se pone NULL
                return null;
            }
            else 
            {
